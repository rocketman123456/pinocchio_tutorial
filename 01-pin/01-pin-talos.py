...
constraint_models[2].joint2_placement = pinocchio.SE3(pinocchio.rpy.rpyToMatrix(np.array([0, -np.pi / 2, 0.0])), np.array([0.6, 0.4, 1.0]))
...
def squashing(model, data, q_in):
    ...
    for k in range(N):
        ...
        J = np.vstack([pinocchio.getFrameJacobian(model, data, cm.joint1_id, cm.joint1_placement, cm.reference_frame) for cm in constraint_models])
